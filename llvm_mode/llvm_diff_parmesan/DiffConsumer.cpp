...
void DiffConsumer::header() {
  ...
  for (SmallVectorImpl<DiffContext>::iterator
    ...
    if (isa<Function>(I->L)) {
      ...
      const collabfuzz::IDAssigner::IdentifiersMap *IdMap = &IdAssigner->getIdentifiersMap();
...
void DiffConsumer::printStats() {
  ...
  // Print Cmp IDs
  collabfuzz::IDAssigner::CmpsMap cmps_bb = IdAssigner->getCmpMap();
  std::set<collabfuzz::IDAssigner::CmpIdType> cmps;
...
void DiffConsumer::setIdAssigner(const collabfuzz::IDAssigner *assigner) {
    IdAssigner = assigner;
}
